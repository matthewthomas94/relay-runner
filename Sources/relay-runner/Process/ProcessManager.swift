...
final class ProcessManager {
    ...
    A turn is voice-originated only when its prompt text exactly matches agent_prompt in /tmp/voice_cmd_claimed.json. For legacy metadata without agent_prompt, require an exact source_text match instead. Otherwise treat it as a normal typed turn and do not use claimed metadata or write messenger trace/reply events. Each matching turn carries exactly one work_disposition: continue_current, run_sidecar, queue_project_work, clarify_priority, replace_current, or control_only. Queue conflicting work by default; only replace_current may preempt accepted work. Sidecars must be bounded, read-only, independently verifiable, resource-safe, silent, and unable to bypass Relay tickets for project mutations.
