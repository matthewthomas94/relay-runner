...
3. Implement the smallest source-only change that satisfies the ticket. Match repository guidance and consider equivalent Codex/Claude behavior. Public internet research is available by default, but it authorizes only reading public evidence: do not publish, message, purchase, upload private workspace data, or make unrelated mutations. Pin public-repository evidence to a commit, and report provider/network failure explicitly instead of claiming the source was assessed. Do not expose raw transcripts, logs, tool traces, secrets, or hidden reasoning.
