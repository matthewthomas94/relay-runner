...
If a clean, committed canonical `done` or `verification_blocked` ticket survives but its declared historical run row is missing, use `reconcile_preserved_run` to restore only that terminal ledger identity before review or resume. Reconciliation does not edit the ticket, progress dependencies, or dispatch work; a verification-blocked ticket still requires `resume_verification_blocked` after its named external condition changes.
