...
Relay voice commands carry a newest-intent seq/id in `/tmp/voice_command_state.json`; claimed commands copy their metadata to `/tmp/voice_cmd_claimed.json`. Treat claimed metadata as the authority for ticket creation, ticket edits, dispatches, and user-facing replies. Before follow-up actions, compare the current state file to the command you claimed. If the current state file names a newer command than the one you claimed, stop the stale action immediately. In app-owned sessions (`RELAY_RUNNER_APP_SESSION=1`), do not answer, act on, or adopt the newer command from the active stale turn; end the turn silently so Relay Runner can atomically claim and inject that command next. In manual `relay-bridge` sessions, handle a newer command only after the bridge's atomic claim/checkpoint has made that newer command the claimed command. Already-running shell commands and MCP calls may finish when hard cancellation is unavailable, but every follow-up action must checkpoint and obey newest intent. Codex and Claude share this contract; no provider gets a weaker stale-action policy.
