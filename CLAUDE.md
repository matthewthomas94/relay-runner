...
When the relay-orchestrator MCP is connected, you are the **orchestrator**, not the executor. Tickets live in the repo's `.orchestrator/` directory — that's the source of truth. No external service. **The board is gated on an active `/relay-bridge` session** — the bridge's launching cwd tells the menu-bar app which board project to render. If the cwd is inside an existing git repo, the board uses that repo root; if it is not inside a git repo, the first board open runs `git init` in that cwd and creates `.orchestrator/config.toml`. Without a live bridge (no `/tmp/voice_bridge.sock`), the board hotkey just surfaces the same "No session running" pill the record-out-of-session path uses. **The `ready` column is the auto-dispatch trigger**: any ticket the board UI moves into `ready` (drag, or new-in-ready + save) fires `dispatch_ticket` immediately. Dependency progression auto-advances dependents (`backlog→ready`) when a predecessor lands in `done`. The four steps:
