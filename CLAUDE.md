...
Relay voice commands carry a newest-intent seq/id in `/tmp/voice_command_state.json`; claimed commands copy their metadata to `/tmp/voice_cmd_claimed.json`. Treat claimed metadata as two separate contracts. For user-visible replies, traces, fallback completions, and TTS, compare the current state file to the command you claimed; if a newer command exists, stop stale output and let Relay Runner deliver the newer turn. For ticket creation, ticket edits, dispatches, and other project mutations, pass the claimed `relay_command_seq` and `relay_command_id`; the daemon allows an older command only when Relay Runner registered that bounded mutation authorization and no replacement, redirect, interrupt, or cancel turn has revoked it. Acknowledgement, inspection/status, and additive turns may become the newest conversation without revoking prior authorized mutations. In app-owned sessions (`RELAY_RUNNER_APP_SESSION=1`), do not answer, act on, or adopt the newer command from the active stale turn; end the turn silently so Relay Runner can atomically claim and inject that command next. In manual `relay-bridge` sessions, handle a newer command only after the bridge's atomic claim/checkpoint has made that newer command the claimed command. Already-running shell commands and MCP calls may finish when hard cancellation is unavailable, but every follow-up output or mutation must checkpoint against the correct contract and report any actions that already started or were canceled. Codex and Claude share this contract; no provider gets a weaker stale-action policy.
