---
id: RR-228
artifact_id: ticket-f3c7b61955460c6502ef33f90d66bcfd03b435eb
activity_at: 2026-07-26T06:33:47.450270Z
...
**Run 5** (attempt 2) - branch `relay/rr-228`

- Fixed `BoardOverlayScrollContainer` to account for settled hosted AppKit
  descendants in scrollable lanes, with a top-endpoint correction for manual
  input and keyboard scroll handling through the same clip-view clamp.
- Added a mounted `ProgramWorkColumnPanel` regression covering Backlog and Done
  lanes, precise wheel events, line wheel events, Home-key scrolling, bottom
  reachability, first-card frame visibility, and thumb top placement.
- Verification passed: `swift test --filter BoardOverlayScrollViewTests` and
  `swift test` (541 tests). Installed-app live Workspace verification was not
  run from this isolated worker; reviewer should perform that visual check.
