---
id: RR-213
artifact_id: ticket-6adfa7cd339d04b605e57282b6395a41cb44e47f
activity_at: 2026-07-24T22:34:33.775505Z
...
### Run 314 (attempt 1) — branch `relay/rr-213`

- Added an active-ticket SwiftUI/AppKit glow host that reuses `ParticleFieldRenderer` with the TTS theme behind solid Workspace ticket cards, gated by `ProgramStatusItem.hasActiveWorker`.
- Added a shared particle animation clock so multiple active cards reuse one scheduler while keeping independent renderer/mask callbacks, and added lazy cleanup for inactive/reused card views.
- Added tests for active-worker versus lane-only gating, mask evolution and bounds, shared-clock lifecycle, multiple active cards, and effect sizing; `swift test --skip-update` passed 504 XCTest tests.
- Status: done. Visual reference attachment was inspected before implementation; no separate app screenshot capture was produced in this worker run.
