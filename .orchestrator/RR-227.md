---
id: RR-227
artifact_id: ticket-5d6583527cadfb158294eb4916d83a71c83abc63
activity_at: 2026-07-26T03:22:18.121104Z
...
### Run 1 (attempt 1) - branch `relay/rr-227`

- Added the current top ticket identity to each `ProgramWorkColumnPanel` scroll reset key so same-scope refreshes that prepend or replace the first ticket return that lane to the true top without affecting other lanes.
- Added a regression that mounts the real Done work column with 212 variable live-style ticket cards, preserves a stale positive offset, then verifies a same-scope prepended top ticket resets to `minY == 0` and the thumb is at the visual top.
- Verification: `swift test --filter BoardOverlayScrollViewTests` and full `swift test` both passed. Manual live Workspace visual verification was not performed from this isolated worker.
