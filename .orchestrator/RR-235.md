---
id: RR-235
artifact_id: ticket-1c02a08ee541a1f03061f0b7888034100355a9be
activity_at: 2026-07-27T08:17:55.431427Z
...
### Run 2 (attempt 2) — branch `relay/rr-235`

- Added shared on-disk onboarding-state gating so ordinary Workspace entry points are suppressed across app instances during fresh first-run and manual-redo onboarding, while the tutorial-only bypass remains explicit.
- Updated Workspace dashboard loading so polling does not restart an in-flight first load, and hide/suspend/deinit cancel model reloads and clear reveal loading state.
- Added regression tests for shared onboarding flags, AppState shell locking, and slow dashboard reload polling; focused tests and full `swift test` passed.
