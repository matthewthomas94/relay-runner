...
2. **Refined ticket authoring.** Raw Relay command captures are private metadata, not ticket prose. The persistent orchestrator state receives that metadata in parallel for durable tracking, while the PM frontstage creates or refines visible tickets under `<repo>/.orchestrator/<TICKET_ID>.md` as management work. Ticket prose must be actionable and user-safe rather than raw transcript text. Each ticket needs a title, a `## Description`, `## Acceptance criteria`, a priority, sizing fields, and `depends_on` when needed.
