...
The user experiences Relay Runner as a three-layer loop: the **PM frontstage** talks to the user and reports status, the **persistent orchestrator** owns durable planning and ticket shaping, and **workers** execute individual tickets in isolated worktrees. Tickets live in this repo's `.orchestrator/` directory — that's the source of truth. No external service. **The board is gated on an active `/relay-bridge` session** — the bridge's launching cwd is what tells the menu-bar app which repo's `.orchestrator/` to render, so without a live bridge (`/tmp/voice_bridge.sock` present) the board hotkey just surfaces the same "No session running" pill the record-out-of-session path uses. **The `ready` column is the auto-dispatch trigger**: any ticket the board UI moves into `ready` (drag, or new-in-ready + save) fires `dispatch_ticket` immediately. Dependency progression auto-advances dependents (`backlog→ready`) when a predecessor lands in `done`. The loop is:

1. **PM acknowledgement and discovery.** The PM frontstage handles the live conversation, asks clarifying questions, reports outcomes, and keeps the user updated. This is the same Karpathy "Think before coding" beat from `~/.codex/AGENTS.md`, but the PM does it while the persistent orchestrator keeps background state.
2. **Persistent orchestrator ticket authoring.** Raw Relay command captures are private metadata, not ticket prose. The persistent orchestrator receives that metadata in parallel, resolves the target project, writes or refines visible tickets under `<repo>/.orchestrator/<TICKET_ID>.md`, and keeps them cold-start safe for workers. Each ticket needs a title, a `## Description`, `## Acceptance criteria`, a priority, sizing fields, and `depends_on` when needed.
