...
The user experiences Relay Runner as a three-layer loop: the **PM frontstage** talks to the user, reports status, and manages refined ticket prose; the **persistent orchestrator state** privately records raw Relay metadata and durable session status; and **workers** execute individual tickets in isolated worktrees. Tickets live in this repo's `.orchestrator/` directory — that's the source of truth. No external service. **The board is gated on an active `/relay-bridge` session** — the bridge's launching cwd is what tells the menu-bar app which repo's `.orchestrator/` to render, so without a live bridge (`/tmp/voice_bridge.sock` present) the board hotkey just surfaces the same "No session running" pill the record-out-of-session path uses. **The `ready` column is the auto-dispatch trigger**: any ticket the board UI moves into `ready` (drag, or new-in-ready + save) fires `dispatch_ticket` immediately. Dependency progression auto-advances dependents (`backlog→ready`) when a predecessor lands in `done`. The loop is:

1. **PM acknowledgement and discovery.** The PM frontstage handles the live conversation, asks clarifying questions, reports outcomes, and keeps the user updated. This is the same Karpathy "Think before coding" beat from `~/.codex/AGENTS.md`, while persistent orchestrator state records background metadata and dispatch lifecycle.
2. **Refined ticket authoring.** Raw Relay command captures are private metadata, not ticket prose. The persistent orchestrator state receives that metadata in parallel for durable tracking, then the backstage command loop or PM frontstage creates/refines visible tickets under `<repo>/.orchestrator/<TICKET_ID>.md` through structured management actions. Ticket prose must be actionable and user-safe rather than raw transcript text. Each ticket needs a title, a `## Description`, `## Acceptance criteria`, a priority, sizing fields, and `depends_on` when needed.
3. **Worker creation and execution.** When a ticket is concrete enough to hand to a colleague cold, move it to `ready` or call `mcp__relay-orchestrator__dispatch_ticket`. The persistent orchestrator dispatches a worker in an isolated worktree. The PM frontstage reports run ids, board changes, failures, and completion status to the user without becoming the worker.
4. **Orchestrator review and PM status update.** Each worker commits code plus its ticket update on `relay/<id>`. The persistent orchestrator reviews and integrates those branches in a sensible order, publishes `done` by merging, and the PM frontstage reports the outcome and any next step to the user.
