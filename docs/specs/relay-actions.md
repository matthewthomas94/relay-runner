...
9. **Screen Recording permission flow**: First-run UX explains and routes the user to grant Screen Recording — to the *terminal*, not Relay Runner.
   ...
   - Target: New `PermissionKind.screenRecording` case. `Info.plist` includes `NSScreenCaptureUsageDescription`. PermissionsManager polls Screen Recording status via the same pattern. The app degrades gracefully if denied — `screenshot` tool returns a clear error string and the user is notified once via the existing `PermissionNotifier`.
   - **Terminal attribution caveat**: macOS attributes Screen Recording to the *responsible* process. Because `relay-actions-mcp` is spawned by the agent CLI (which is spawned by the user's terminal/IDE), the TCC prompt fires for **the terminal or IDE**, not Relay Runner. The screenshot-tool error message uses `ParentProcess.detectTerminal()` to walk the process tree and name the actual app the user must grant (Terminal / iTerm / Warp / VS Code / Cursor / etc.). Granting Relay Runner is harmless and recommended for future menu-bar-driven tools, but does not fix today's prompt.
   - Acceptance: On a fresh install, denying Screen Recording does not crash the app or block voice features; the perimeter overlay does not appear; calling the `screenshot` tool from voice returns a TTS message that names the terminal/IDE the user must grant and the relaunch step required.
...
- **TCC attribution to terminals.** Screen Recording (and Accessibility, for CGEvent) prompt against the *responsible* parent process — typically the terminal (Terminal / iTerm / Warp) or IDE (VS Code / Cursor) the user launched the agent CLI from, not Relay Runner. Means the user grants Screen Recording N times if they use N terminals. The screenshot-tool error message names the specific app via `ParentProcess.detectTerminal()`. A future v1.1 could: (a) detect first-launch and proactively prompt with a friendly modal naming the parent, (b) maintain a "known-good terminals" list in app config to skip the warning on subsequent runs.
