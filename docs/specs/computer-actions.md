...
9. **Screen Recording permission flow**: First-run UX explains and routes the user to grant Screen Recording — to the *terminal*, not Relay Runner.
   ...
   - **Terminal attribution caveat**: macOS attributes Screen Recording to the *responsible* process. Because `relay-actions-mcp` is spawned by `claude` (which is spawned by the user's terminal/IDE), the TCC prompt fires for **the terminal**, not Relay Runner. The screenshot-tool error message uses `ParentProcess.detectTerminal()` to walk the process tree and name the actual app the user must grant (Terminal / iTerm / Warp / VS Code / Cursor / etc.). Granting Relay Runner is harmless and recommended for future menu-bar-driven tools, but does not fix today's prompt.
