...
The synchronizer first reconciles the selected remote to the preview base using only the exposure-confirmed fetch and push URL digests. It then normally fast-forward-pushes only the exact prepared artifact refspec, creates a second fresh quarantine, refetches only that ref through the confirmed fetch URL, and proves that the prepared commit is reachable and the catalog, ticket, and attachment blob identities and SHA-256 digests match. An indeterminate push is resolved by refetching. A remote descendant that still reaches the prepared commit is safe; unrelated, missing, protected, non-fast-forward, offline, authentication, shallow-object, and integrity outcomes remain retryable blockers. No force path exists.
