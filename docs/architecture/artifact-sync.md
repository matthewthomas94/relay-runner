...
Retention publication has stricter confirmed-sync and prepared-commit entry points. Archive preflight, prepared publication, and post-push verification resolve and use only the exposure-confirmed fetch and push URL digests. Prepared publication leaves the local artifact authority unchanged, pushes one exact artifact refspec, then performs a second quarantine fetch and verifies caller-supplied source-commit/path/blob/SHA-256 proofs. Push success alone is never durability evidence. Fresh- and second-device recovery require the confirmed fetch URL digest, fetch only the configured artifact ref, validate a caller-supplied retention-layout predicate before adoption, and refuse remote retargeting, an existing unowned or manually edited materialization, or divergent local artifact history.
