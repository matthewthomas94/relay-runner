...
Program capture-only records are exported to immutable artifact events before a clean registry-v2/artifact/run-backed Graphify rebuild. The final step writes, but does not execute, a versioned `terminal-count-v1` retention preview. It reports every nonterminal materialization, the ranked and retained terminal IDs, exact eviction candidates, temporary blockers, transaction state, and rollback materialization IDs. Missing canonical activity fails closed. The journal records verified artifact/source commits, the manifest digest, backups, remote outcome, Program rebuild, retention plan, and exact rollback instructions. Re-running the same command resumes or returns the completed result without duplicating commits.
