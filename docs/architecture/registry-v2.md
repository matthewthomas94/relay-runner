...
The v2 service is the default Workspace and session-scope path. Setting `RELAY_RUNNER_REGISTRY_V2=0` (or any value other than `1`, `true`, `yes`, or `enabled`) restores the preserved legacy read path without deleting v2 state. Phase 1 established storage and phase 2 moved Workspace and session scope into the service; the [explicit scope contract](project-scope-v2.md) documents that boundary. The legacy file is never changed or removed by v2 migration.

This boundary makes rollback non-destructive: the v2 JSON, backup, and bookmark files remain intact while the legacy path is active, and rollback must not touch a registered repository. The implementation does not yet initialize `relay/artifacts`, sync a remote, or cut over repository-local `.orchestrator` files.
...
| `projects/bookmarks/<project-id>.bookmark` | App-owned regular bookmark used to resolve a registered project after launch or relocation |
...
Each JSON record contains the immutable `project_id`, display name, user-selected and last-resolved paths, file-resource identifier, Git common-directory fingerprint, primary or user-managed worktree kind, remote mode/ref metadata, availability, timestamps, and a bookmark reference. Bookmark bytes are never JSON. They are regular Foundation bookmark files with owner-only permissions under `projects/bookmarks/`. The serialized `com.relayrunner.project-bookmark` namespace remains unchanged for schema-2 compatibility; it no longer names or accesses a Keychain service.
...
- A registry created by an earlier Keychain-backed build has no bookmark file. On first use, Relay Runner validates the recorded selected or last-resolved path against the Git common directory or committed project ID before writing a regular bookmark file. It never reads or deletes the legacy Keychain item, so migration cannot show a Keychain password prompt.
- A missing path remains registered as `missing`; a missing `/Volumes/<name>` mount is `offline`.
- A missing bookmark that cannot be rebuilt from a validated recorded path, or a stale, revoked, or unreadable bookmark, becomes `access_requires_regrant`. Stale bookmark bytes are never refreshed silently. Locate/regrant validates identity before explicitly replacing the bookmark file.
...
Legacy path records migrate in stable ID order. Existing artifact IDs are adopted; otherwise the legacy record ID deterministically derives an immutable v2 ID. Migration is idempotent, preserves the legacy file and active-project mapping, and records no bookmark until the first identity-validated use materializes one. Missing records remain in the registry for Locate or Remove.

Remove requires explicit confirmation. It unregisters the record, removes its bookmark file, and deletes only `indexes/<project-id>/` and `caches/<project-id>/`. It never edits or deletes repository files, `.orchestrator`, refs, remotes, the Git index, source history, or remote branches.

## Platform and provider behavior

Regular Foundation bookmarks are a macOS-only relocation aid, not a filesystem permission or credential. Relay Runner is not App Sandbox constrained, so project bookmark persistence stays in app-owned Application Support and never uses Keychain. All project identity, duplicate, availability, migration, and removal semantics are provider-neutral: Codex and Claude use the same records and validation. Provider-specific executable discovery, authentication, model naming, and effort flags remain outside registry v2.
