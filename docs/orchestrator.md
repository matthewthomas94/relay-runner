...
Messenger model choices use stable provider aliases. The lightweight defaults are Codex `luna` at low effort and Claude `haiku`; Codex resolves the newest account-visible concrete model from the local `app-server` catalogue, while Claude uses its warm stream-JSON process. Both providers stream the first complete semantic sentence to the speech path, disable tools and MCP access, and continue foreground delivery independently. Optional `[general]` keys are `messenger_enabled`, `messenger_model`, and `messenger_effort`. Privacy-safe command timing is written to the speech event log from user-turn receipt through Messenger submission, provider start/output, TTS enqueue, and playback; `scripts/speech-latency-report.py` attributes the first missing stage and passes the installed UAT gate only when at least nine of ten samples meet both targets with authenticated output from Codex and Claude. Parakeet STT and Kokoro TTS remain local and unchanged.
...
For `execution_mode: spike`, dispatch takes a separate path: the daemon creates a detached read-only clone without a `relay/<id>` branch, supplies immutable local repository/ticket evidence, enables provider-native public web research, and requires structured conclusions, evidence, uncertainties, recommendations, mutation-attempt reporting, and an explicit research-access status. Codex uses its native `--search` tool with the local command sandbox kept read-only; Claude runs in safe mode with `Read`, `Glob`, `Grep`, `WebSearch`, and `WebFetch`. A valid result is committed directly to the ticket as `## Spike report` and reaches Done without an implementation review or merge. Failure or cancellation cleans the snapshot, records the exact cause, and returns the ticket to Backlog for explicit retry. Backlog dependents are not auto-promoted by spike completion; already-authorized Queued dependents remain eligible.

Before a Codex spike starts, the macOS daemon selects a direct Git executable from PATH, the selected Xcode/Command Line Tools installation, or standard standalone Git locations. Homebrew is optional. Apple's `/usr/bin/git` launcher is excluded because even a read can attempt an xcrun cache write. A bounded preflight denies filesystem writes and network access while verifying HEAD/file-list reads in non-login sh, bash, zsh, and the inherited shell. Diagnostics reject a candidate even if it exits zero. If no candidate passes, dispatch fails before starting the worker, records a repair-and-retry explanation, and returns the ticket to Backlog.

The selected executable is available as `$RELAY_SPIKE_GIT` and first on PATH, with optional Git locks and lazy fetching disabled. The daemon supplies these settings to both the worker process and Codex command tools; Codex spikes disable login shells, shell snapshots, profile environment loading, and approval escalation. Shell startup must preserve the prepared environment. Workers must not invoke `/usr/bin/git` or xcrun, warm caches, install tools, or retry outside the sandbox. No writable temporary acquisition directory is added: provider web tools return evidence directly to model context, while the source/ticket snapshot stays read-only and follows the existing bounded cleanup path. All public research uses native provider web tools. Codex command networking stays disabled by the OS sandbox, including interpreters and child processes; there is no shell-download fallback. Command-denial events always fail the spike and never become native research-access errors. Command-text checks provide diagnostics only and grant no permissions. Claude deliberately has no shell or Git command tool, so it needs no Git preflight. Both providers inspect public repositories through exact HTTPS commit/raw-file URLs pinned to a full commit SHA. See [RR-355 validation](investigations/RR-355-spike-git-toolchain.md).

For artifact-backed projects, the daemon publishes the claim and terminal report through the canonical artifact writer. The detached snapshot includes the immutable assigned ticket, its attachments, dependency summaries, and a manifest checked before report publication. Codex and Claude share this lifecycle and result contract; their existing sandbox and output-schema mechanisms remain provider-specific. Retrieved content is untrusted evidence and cannot expand tool permissions. Search queries and public URLs must not contain private workspace content, local paths, ticket text, meeting data, credentials, or secrets. `research_access: failed` must contain the exact provider/network error and a matching uncertainty; it is invalid to claim that an unavailable public source was assessed. A supported no-go or further-evidence conclusion is successful research when accompanied by a valid report.

Default research access is role-scoped:

| Role | Codex | Claude | Mutation boundary |
| --- | --- | --- | --- |
| Foreground orchestrator | Native `--search` plus normal tools | Default built-in tools, including WebSearch/WebFetch | Existing user and project authorization still governs every mutation. |
| Implementation and review workers | Native `--search` plus isolated-worktree tools | Default built-in tools, including WebSearch/WebFetch | Research adds no publishing, messaging, private-data upload, or unrelated-write authority. |
| Research spike | Native `--search`; local commands remain read-only | Read/Glob/Grep plus WebSearch/WebFetch; no shell | Only public reads and the terminal structured result are allowed; the daemon alone persists the report. |
| Bounded sidecar | Web search enabled; shell, apps, MCP, and repository tools disabled | WebSearch/WebFetch only | No project, desktop, or external mutation. |
| Messenger and continuity agent | No tools | No tools | Intentionally tool-free; no research access. |

See [RR-357 validation](investigations/RR-357-default-research-access.md) for the reproducible pinned-repository smoke, provider evidence, and installed-runtime rollout boundary.

Artifact publication precedes `SpikeCompleted` in the run ledger. Duplicate delivery and startup recovery reuse the run's canonical event. If publication is interrupted or conflicts with another artifact edit, the daemon retains the snapshot and lease, reports the persistence error, and retries during startup reconciliation. A committed report can repair the board projection and run ledger even if local output has already been cleaned up. No source commit, implementation review, or merge is involved.

Startup also publishes failed or canceled spike outcomes when the worker stopped before its completion callback. Publication retries preserve the terminal state and original diagnostic; separate run activity describes the persistence error. The snapshot lease remains pinned until the canonical outcome is published, including across repeated restarts.
...
The daemon reconciles this state from board sweeps, worker completion callbacks, review-worker callbacks, merge decisions, cancellation, startup recovery, and a bounded monitor loop. It does not keep Codex or Claude spending tokens to poll unchanged state. Stale implementation ownership is marked `Stalled` and recovered through the same backoff-protected auto-dispatch path; stale review ownership returns to `AwaitingReview` and gets a review worker again. Transient failures keep retrying after the backoff until the ticket reaches a terminal outcome or is canceled. Deterministic failures, merge conflicts, missing authority, broken dependencies, and capacity waits remain visible blockers instead of becoming silent loops.
...
Direct computer requests remain with the foreground PM. They do not become tickets and are not sent to the daemon or a worker. The PM prefers deterministic shell or operating-system commands for app launch, known-path open/reveal, and file lookup; it uses Relay Vision when visible pixels are required and Relay Actions only when simulated interaction is necessary. The messenger remains tool-free and speaks the PM's result.
...
For artifact-backed projects, select one project in Workspace and open **History** to search terminal records that are no longer materialized. Viewing detail never recreates Markdown. Restore is explicit; reopening moves the ticket to Backlog and therefore into the uncapped unfinished set. The adjacent **Storage** preview keeps all unfinished tickets, retains one combined pool of the 25 newest Done or Canceled tickets, and removes candidate Markdown or Relay-owned attachments only after confirmed publication and refetch verification. The preview accounts for reachable Git objects, archive metadata, indexes, caches, databases, and run logs separately because retention does not purge them.
