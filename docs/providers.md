...
Claude spikes expose only `Read`, `Glob`, `Grep`, `WebSearch`, and `WebFetch` in safe mode, with no shell or Git command access and no Git preflight requirement. Codex uses native web search for all public research; its local commands remain in the read-only, network-disabled OS sandbox. There is no shell-download fallback or shell-command permission classifier. Both providers inspect public repositories through commit/raw-file URLs pinned to a full commit SHA and report unavailable native research access explicitly. Immutable snapshot inputs, rejection of denied commands, and daemon-only structured report persistence remain unchanged. See [spike execution](orchestrator.md) for the full contract.
